...
**Версия 0.1.8 · Windows x64 · Активная разработка · Приоритет: качество изображения**

[Скачать Windows-версию](https://github.com/AlexCodeHadjar/matvision-studio/releases/tag/v0.1.8) · [Инструкция](docs/USER_GUIDE.md) · [План развития](ROADMAP.md) · [Сравнение с аналогами](docs/COMPARISON.md) · [Сборка](DEVELOPMENT.md)
...
| Фото     | Трассировка путей, 10 отражений, до 4096 проходов, DOF, опциональный denoise и рельеф по карте высоты    |
| Ultimate | Локальный Cycles prototype через отдельный Blender 4.5 LTS: плоский коврик, принт, камера, PNG и отмена  |
...
В 0.1.7 Photo получил режимы Preview/High/Ultra/Reference, физическую камеру без размытия по умолчанию, встроенный пространственный фильтр шума и ограниченный геометрический рельеф для отдельного снимка. **Ultimate пока прототип:** Cycles рендерит только базовый плоский коврик и предупреждает о непренесённых шве, изгибе и картах материалов. Нужен отдельно установленный или распакованный Blender 4.5 LTS; он не входит в установщик. [Матрица возможностей и ограничения](docs/RENDERERS_0.1.7.md).

В 0.1.8 исправлено сохранение готового PNG Ultimate в Windows-приложении: кнопка становится доступной только после загрузки изображения, открывает системный диалог и сообщает результат рядом с собой.
...
![Прототип Ultimate с результатом Cycles в Windows-приложении](docs/images/ultimate-prototype-0.1.7.png)

_Ultimate 0.1.7: реальный результат Blender/Cycles, базовая плоская сцена. Шов, изгиб и карты материалов пока не перенесены._

![Photo 0.1.7 с новым шумоподавлением и f/16](docs/images/photo-smartdenoise-0.1.7.png)

_Нативный Photo-прогон 0.1.7: 128 проходов, SmartDenoise, f/16. Сопоставление экспозиции с Ultimate ещё не завершено._
...
1. Откройте [релиз v0.1.8](https://github.com/AlexCodeHadjar/matvision-studio/releases/tag/v0.1.8), скачайте `MatVisionStudio-0.1.8-x64-setup.exe` и установите программу.
...
Репозиторий содержит исходники 0.1.8, тесты, документацию и ресурсы. Установщик размещается в Releases; Blender, зависимости сборки, кеши и личные проекты в Git не включаются. [Проверка Windows-версии 0.1.7](docs/release/native-quality-0.1.7.json) фиксирует прогоны Photo и Cycles prototype; [проверка исправления сохранения 0.1.8](docs/release/native-quality-0.1.8.json) фиксирует отдельный нативный прогон. [Матрица рендереров](docs/RENDERERS_0.1.7.md) перечисляет ещё не реализованный перенос и калибровку. Отдельная лицензия собственного кода ещё не оформлена; лицензии зависимостей сохраняются.
